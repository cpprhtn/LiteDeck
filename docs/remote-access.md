...
돈은 들지 않습니다. 개인이 자기 기기를 묶는 정도는 무료 플랜으로 충분합니다
([요금제](https://tailscale.com/pricing) — 조건은 바뀔 수 있으니 숫자는 저쪽을 보세요).
...
둘 중 하나로 통일하는 편이 낫습니다. 서버의 sshd를 그대로 쓸 생각이면 Tailscale SSH는
꺼 두세요.
...
- **MCP 엔드포인트는 tailnet에 올라가지 않습니다.** 노트북 외부로 나가지 않으므로, tailnet의
  다른 기기가 이 엔드포인트를 통해 서버를 조회하거나 바꿀 수 없습니다
- **서버에는 여전히 아무것도 설치하지 않습니다.** Claude Code를 집 PC에 깔아 tailnet으로
...
집에 서버가 여러 대인데 전부에 Tailscale을 깔고 싶지 않다면, 한 대를
[서브넷 라우터](https://tailscale.com/kb/1019/subnets)로 두고 그 대역을 광고하면 됩니다.
나머지는 원래 LAN IP 그대로 LiteDeck에 등록합니다. 그 서버들 입장에서는 **Tailscale이라는
것이 존재하지 않고**, LiteDeck도 차이를 모릅니다.

LiteDeck의 원칙 1이 여기서도 유지됩니다. 설치물은 라우터 한 대에만 생기고 나머지 서버는
그대로입니다.
...
## VPN 없이: 이미 닿는 서버가 한 대 있다면

집이나 회사에 **밖에서 SSH로 닿는 서버가 이미 한 대** 있다면 VPN을 새로 세울 일이 아닙니다.
그 서버를 경유지로 삼으면 같은 네트워크의 나머지 기기에 닿습니다. 호스트 편집기의
**ProxyJump** 칸에 경유 서버를 적으면 됩니다.

```
호스트      192.168.0.20        ← 안쪽 기기. 밖에서는 안 보입니다
ProxyJump   me@myserver.com     ← 밖에서 닿는 서버
```

경유 서버도 **로그인하는 서버로** 다룹니다. 지문을 따로 확인하고 비밀번호도 따로 묻습니다.
목적지의 호스트 키 검사는 그대로입니다 — 경유 서버는 바이트를 전달할 뿐이고, 반대편이
누구인지 보증하지 않습니다.

경유 서버의 sshd에 `AllowTcpForwarding yes`가 필요합니다. 한 단계만 지원하고, 여러 단계를
적으면 첫 단계로 줄이지 않고 거부합니다.

**Tailscale과 비교하면** — 새로 깔 것이 없고 계정도 필요 없지만, 밖에서 닿는 서버가 이미
있어야 하고 그 포트가 인터넷에 열려 있어야 합니다. 그게 싫어서 이 문서를 보고 계신 거라면
Tailscale 쪽이 맞습니다.
...
**집 PC가 잠들면 접속되지 않습니다.** 절전을 끄는 것이 가장 확실합니다. Wake-on-LAN은
매직 패킷을 **같은 LAN 안에서** 쏴야 해서, 밖에서 tailnet을 타고 직접 깨울 수는 없습니다.
그 LAN에 상시 켜져 있는 다른 기기가 있고 거기에도 Tailscale이 깔려 있다면, 그 기기로
들어가서 쏘는 방법은 됩니다. 이건 LiteDeck이나 Tailscale이 해결해 주는 문제가 아닙니다.
