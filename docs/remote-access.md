...
합치면 **포트포워딩도 서버 설치물도 없이** 카페에서 홈서버의 파일을 열고, 서비스를
...
[tailscale.com/download](https://tailscale.com/download)에서 받아 홈서버와 노트북 양쪽에
설치하고, **같은 계정으로** 로그인합니다. 그러면 두 기기가 같은 tailnet에 들어갑니다.

공유기는 건드리지 않습니다. 포트를 열지도, DDNS를 붙이지도 않습니다.

## 3. 홈서버의 tailnet IP 확인
...
접속을 누르면 처음 한 번 호스트 키 지문을 확인합니다. **이 지문은 홈서버에서 직접
확인하세요** — Tailscale이 연결 경로를 만들어줄 뿐, 상대가 맞는지 검증하는 것은
여전히 SSH의 일입니다.

```bash
# 홈서버에서 실행해 LiteDeck이 보여주는 지문과 대조합니다
ssh-keygen -lf /etc/ssh/ssh_host_ed25519_key.pub
```

여기까지가 전부입니다. 이제 노트북 어디서든 홈서버가 사이드바에 뜹니다.
...
```
Claude Code ──로컬 HTTP──▶ LiteDeck ──Tailscale(WireGuard)──▶ 홈서버
  (노트북)                  (노트북)                            (설치물 없음)
```

- **MCP 엔드포인트는 tailnet에 올라가지 않습니다.** 노트북 외부로 나가지 않으므로, tailnet의
  다른 기기가 이 엔드포인트를 통해 서버를 조회하거나 바꿀 수 없습니다
- **서버에는 여전히 아무것도 설치하지 않습니다.** Claude Code를 홈서버에 깔아 tailnet으로
...
**홈서버가 잠들면 접속되지 않습니다.** 절전을 끄는 것이 가장 확실합니다. Wake-on-LAN은
...
**어디까지 해봤는지.** 위 1~4번 — tailnet 주소로 호스트를 등록해 붙고, 파일과 지표를
읽는 것까지는 실제로 확인했습니다. 나머지 절은 아직입니다: **Tailscale SSH, 서브넷
라우터, Headscale, WireGuard 직접 구성**은 [지원 범위](support.md)의 기준대로
**검증되지 않은 것은 검증되지 않았다고** 적어 둡니다. 해보셨다면 결과를
