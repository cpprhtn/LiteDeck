...
> [!IMPORTANT]
> **릴리스의 Linux 바이너리는 `libwebkit2gtk-4.1` 에 링크돼 있습니다.** CI 러너가 Ubuntu 24.04 라
> 거기 있는 패키지로 빌드되기 때문입니다. **22.04 에서는 실행되지 않습니다** — 4.0 만 있는
...
| **ProxyJump 실서버** | 컨테이너 두 대(경유 + 목적지)로 확인. 실제 bastion 으로는 안 해봄. 다단은 미구현이고 거부합니다 |
