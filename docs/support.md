...
| **NVIDIA GPU 모니터링** | 파서는 실제 `nvidia-smi` 출력으로 검증했고, 드라이버가 멈췄을 때 나머지 지표가 살아남는 것도 확인했습니다. 다만 **저자 환경에 NVIDIA 카드가 없어** 카드가 달린 실기에서는 확인하지 못했습니다. 기여자([@INMD1](https://github.com/INMD1)) 환경에서 동작하는 것을 확인했습니다 |
