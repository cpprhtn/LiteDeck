...
- **MCP도 이 잠금만 씁니다.** 사용자가 보안 탭에서 자물쇠를 열어 둔 동안에는 `svc_control` ·
  `container_control` · `proc_signal`이 root로 돕니다. 비밀번호는 여전히 LiteDeck에만 치고,
  도구의 인자에도 응답에도 들어가지 않으며, **잠금을 여는 도구는 없습니다** — AI가 사람에게
  비밀번호를 묻는 길을 만들지 않으려는 것입니다. 잠금이 닫혀 있으면 앱에서 열라는 문장이
  돌아가고, 그 사이 명령은 로그인 사용자 권한으로 나갑니다
- **MCP 호출은 절대 비밀번호 창을 띄우지 않습니다.** 화면 앞에 사람이 없을 때 뜬 창은 엉뚱한
  이유로 눌리거나, 아무 창에나 비밀번호를 치는 습관을 들입니다
...
- **서버별 opt-in, 파일 삭제와 명령 실행은 각각 또 따로.** 호스트를 등록해도, 읽기를 공유해도 삭제 권한도 명령 실행 권한도 안 생깁니다
...
- **되돌릴 수 있는 것은 파일뿐입니다.** 서비스 재시작이나 프로세스 종료는 사본이 없습니다.
  다시 시작하면 되는 종류라 넣었고, 디렉터리 재귀 삭제와 컨테이너·이미지 삭제는 아예 제공하지
  않습니다
- **명령 실행은 되돌릴 수 없습니다.** `run_command`는 서버별로 따로 켜야 하고(기본 꺼짐),
  켜져 있어도 기본 모드는 매번 물어봅니다. 다만 **「8시간 동안 묻지 않기」를 켜면 명령도 같이
  통과합니다** — 그 모드는 무인 작업을 하라고 켜는 것이고, 명령에서만 멈춰 서면 켠 의미가
  없습니다. 되돌릴 수 없다는 점은 프로세스 종료나 서비스 재시작도 같습니다. 이걸 넣은 이유는
  감사 쪽입니다 — 같은 명령을 터미널 탭에서 치면 Command Log에 아무것도 안 남고, MCP를
  지나면 전부 남습니다. 자세한 근거는 [MCP 문서](mcp.md)에
- **자격증명은 MCP 계층이 만지지 않습니다.** 토큰은 이 엔드포인트에만 쓰이고, SSH 자격증명은
  위에 적은 대로 OS 키체인에 있습니다

## 못 하는 것을 먼저 적어둡니다

- **비밀번호가 메모리에 남는 시간을 완전히는 보장하지 못합니다.** 오래 남는 사본 하나 —
  자물쇠를 연 뒤 연결이 끝날 때까지 들고 있는 sudo 비밀번호 — 는 바이트로 쥐고 잠금을 풀 때
  0으로 덮습니다. 그게 분에서 시간 단위로 사는 유일한 사본입니다. 나머지는 여전히 문자열이라
  지울 수 없습니다: `x/crypto/ssh` 가 인증 순간에 받는 것, 그리고 그것을 건네주려고 만드는 것.
  둘 다 호출 한 번 동안만 살고 바로 가비지가 되지만, **그 순간의 코어 덤프나 힙 덤프에서는
  보일 수 있습니다**
- **릴리스 바이너리는 서명돼 있지 않습니다.** SHA256 체크섬은 서명이 아닙니다
- **어디까지 실기로 확인했는지는 [지원 범위](support.md)에 적어 둡니다.** 라즈베리파이 4 실서버에서 폴더째 전송과 이어받기·sudo 권한 상승 완료·터미널 PTY·실시간 로그까지 확인했고, 윈도우 실서버에서는 세션 끊기와 WSL 콜드 시작까지 확인했습니다. 확인하지 않은 것도 같은 표에 있습니다
