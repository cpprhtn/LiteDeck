...
- **되돌릴 수 있는 것은 파일뿐입니다.** 서비스 재시작이나 프로세스 종료는 사본이 없습니다.
  ...
  않습니다
- **명령 실행은 되돌릴 수 없고, 그래서 다르게 다룹니다.** `run_command` 는 서버별로 따로 켜야
  하고(기본 꺼짐), 켜져 있어도 **승인 모드와 무관하게 매번 물어봅니다.** 「밤새 안 묻기」가
  이 도구에는 적용되지 않습니다. 이걸 넣은 이유는 감사 쪽입니다 — 같은 명령을 터미널 탭에서
  치면 Command Log 에 아무것도 안 남고, MCP 를 지나면 전부 남습니다. 자세한 근거는
  [MCP 문서](mcp.md)에
