...
| **네트워크** | 인터페이스와 열린 포트. **외부에 노출된 포트를 구분해서 표시**. sshd 설정 점검. **127.0.0.1 에만 묶인 Uptime Kuma 를 SSH 터널로 열기** |
...
## Uptime Kuma: 127.0.0.1 에 묶인 것을 열기

Uptime Kuma 를 `127.0.0.1:3001` 에만 묶어 두었다면, 그건 일부러 그렇게 한 것입니다.
대신 볼 때마다 `ssh -L 3001:127.0.0.1:3001 …` 를 손으로 쳐야 합니다.

**이미 열려 있는 세션이 그걸 대신합니다.** 네트워크 탭이 로컬 전용 리스너 중 Kuma 를
찾아내고, **Kuma 열기 (SSH 터널)** 를 누르면 그 세션 위에 포트 포워딩을 걸어 기본 브라우저로
띄웁니다. 새 로그인도, 외워야 할 명령도 없습니다.

**포트가 열려 있다는 것은 뒤에 무엇이 있는지에 대한 증거가 아니라서**, 후보마다 HTTP 요청을
한 번 보내 Kuma 의 화면이 맞는지 확인합니다. 그 요청도 SSH 채널을 그대로 타므로 **이 컴퓨터에는
아무것도 열리지 않습니다.** 확인되지 않으면 **미확인** 으로 표시하고 열기는 그대로 제공합니다 —
사용자가 지정한 포트를 앱이 대신 부정하지는 않습니다.

**터널은 세션과 함께 끝납니다.** 탭을 닫든, 호스트 연결을 끊든, 회선이 끊겨 재연결하든
터널도 같이 닫힙니다. 내 컴퓨터에서 듣고 있는 포트가 응답만 하고 멈춰 있는 상태를 남기지
않기 위해서입니다.

**Command Log 에 남습니다.** 손으로 하면 쳤을 `ssh -N -L …` 이 그대로 적히고, 복사할 수
있습니다. 다만 LiteDeck 은 그 명령을 실행하지 않으므로 — 이미 열린 세션에 채널을 하나 더
여는 것이므로 — 프롬프트 대신 **⇄** 표시가 붙습니다. 실행한 명령인 척하지 않습니다.

**Kuma 대시보드를 여기에 다시 그리지는 않습니다.** Kuma 의 화면은 이미 완성돼 있고, 그것을
요약한 복사본은 원본보다 나쁩니다. Kuma 컨테이너의 재시작·로그·업데이트도 따로 만들지
않았습니다 — 컨테이너 탭이 안에 무엇이 들었는지와 무관하게 이미 합니다.

> [!NOTE]
> 서버의 sshd 에 `AllowTcpForwarding yes` 가 필요합니다. **Alpine 의 openssh 패키지는 이 값을
> `no` 로 배포합니다** (상류 OpenSSH 기본값은 `yes` 입니다). 꺼져 있으면 sshd 가
> `administratively prohibited` 로 거절하는데, 그 문구는 계정 문제처럼 읽히므로 LiteDeck 이
> **어느 설정인지** 를 대신 적습니다.

AI 쪽에서 같은 것을 물어보는 방법은 [MCP 연동](mcp.md)의 `kuma_status` 입니다.
