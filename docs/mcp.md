...
> [!IMPORTANT]
> **파일을 바꿀 때는 기본적으로 물어봅니다.** 승인창에는 실제로 실행될 명령이나 파일 diff가
> 그대로 뜹니다. 그 승인 정책은 **앱이 쥐고 있어 클라이언트 쪽에서는 못 바꿉니다.**
...
| 변경 승인 | 기본은 **파일 변경만 물어봄**. 승인창이 서버 현재 내용 대비 diff를 보여주는데, 그건 클라이언트가 가질 수 없는 정보이기 때문입니다. 재시작 등은 클라이언트가 이미 같은 걸 보여줬으므로 그냥 실행합니다 |
