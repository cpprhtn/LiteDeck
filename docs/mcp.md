...
**명령 실행도 서버별로 따로 켭니다 — 기본은 꺼짐이고, 켜도 매번 물어봅니다.**
...
- **추적이 거꾸로였습니다.** 터미널 탭에서 친 명령은 Command Log 에 **한 줄도 안 남습니다.**
  MCP 를 지나면 도구 호출·실제 명령·종료 코드가 전부 남습니다. 실행을 막는 것이 감사를
  줄이고 있었습니다
- **경계가 이미 명목상이었습니다.** `fs_write` 로 `~/.bashrc` 나 `~/.ssh/authorized_keys` 를
  쓸 수 있고, 그건 다음 접속에 임의 코드가 도는 것과 같습니다

**남는 제약은 되돌리기입니다.** 명령은 사본을 남길 수 없습니다. 그래서 승인 모드 바깥에
둡니다 — 「밤새 안 묻기」를 켜 두어도 `run_command` 는 **매번** 물어보고, 승인창에서
「허용하고 안 묻기」 버튼이 아예 나오지 않습니다. sudo 는 없습니다: 로그인 사용자로 tty
없이 돌기 때문에 비밀번호를 원하는 명령은 기다리지 않고 실패합니다.
...
| 명령 실행은 모드 밖 | `run_command` 는 **승인 모드와 무관하게 매번 물어봅니다.** 되돌릴 사본이 없으므로 「안 묻기」가 기댈 데가 없습니다. 승인창에 그 버튼이 나오지 않습니다 |
