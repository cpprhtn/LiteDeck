...
**되돌릴 수 없는 것은 그대로입니다.** 명령은 사본을 남기지 않으므로 「바뀐 파일」 탭에
나타나지 않고, 승인창도 그렇게 말합니다. sudo 는 없습니다: 로그인 사용자로 tty 없이 돌기
때문에 비밀번호를 원하는 명령은 기다리지 않고 실패합니다.
