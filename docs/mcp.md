...
MCP 패널의 **변경된 파일** 탭에서 파일별로 되돌립니다. 묻지 않기로 두고 자리를 비웠을 때 방어 대신
...
나타나지 않고, 승인창도 그렇게 말합니다. sudo는 없습니다: 로그인 사용자로 tty 없이 돌기
...
| 호스트별 조절 | 헤더의 배지에서 **전부 물어보기 / 파일만 / 8시간 동안 묻지 않기**. 통과 모드는 배지가 붉게 남고 **시간이 지나면 스스로 돌아옵니다** |
