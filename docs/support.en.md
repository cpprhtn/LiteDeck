...
| **File transfers on the Windows client** | Only launching is confirmed; transfers, the terminal and keychain storage are not. **Directory download** gets its own line because it is the one path that behaves differently on this OS: a backslash is an ordinary character in a Linux filename and a path separator on Windows, so a name the server sends could point outside the download folder. **The guard against that is in, and is tested platform-independently.** There is no Windows client here, so **the escape itself was never reproduced** — what was confirmed is that a hostile name reaches the guard and that the guard refuses it |
