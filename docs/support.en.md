...
| **Server** | **Windows 10 Pro 19045 / PowerShell 5.1 (a real SSH server)** | Services, processes, network and monitoring, plus **shell choice in the terminal (cmd, PowerShell, WSL), sessions and security**. Ending a session was checked by opening a throwaway one and watching the client drop; WSL was opened from the app with the virtual machine down. Not a container |
...
| **The Windows security tab without administrator** | Checked only from an administrator account. Each section carries whether it was read and says so when it was not, but which sections a limited account actually loses was never observed |
