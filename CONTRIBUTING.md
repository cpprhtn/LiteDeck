...
예외는 둘뿐입니다. 하나는 보간되는 것이 전혀 없는 **컴파일 타임 상수 스크립트**(`adapter.MetricsScript` 부류) — 사용자 입력이 한 글자라도 섞이면 예외가 아닙니다. 다른 하나는 MCP 의 `run_command`(`mcp_write.go`)인데, 임의의 셸 줄을 실행하는 것이 그 도구의 용도라 조립을 금지할 수가 없습니다. 그래서 서버별로 따로 켜야 하고 기본은 꺼져 있으며 승인을 거칩니다.
