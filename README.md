...
> [!IMPORTANT]
> **조회만 됩니다.** 서버를 바꾸는 도구는 없고, 파라미터로 숨겨져 있지도 않습니다.
> 쓰기는 결재 모델이 먼저 있어야 하는데 아직 만들지 않았습니다.
...
**도구 10개**: `hosts_list` · `health_snapshot` · `sys_stats` · `svc_list` ·
`proc_list` · `container_list` · `net_ports` · `fs_list` · `fs_read` · `sessions_list`.
`health_snapshot` 하나로 CPU·메모리·디스크·failed 유닛·비정상 컨테이너·외부 노출
포트가 한 번에 옵니다. *"myapp 이 500 내는데 원인 봐줘"* 같은 질문 대부분이 이 한 번으로 끝납니다.
...
| 원격에서 못 바꿈 | 그 스위치를 켜는 도구는 없습니다. AI 가 요청해도 앱에서만 바뀝니다 |
