...
| **실제 하드웨어 Linux 서버** | Linux 검증은 컨테이너 픽스처만 사용 |
...
서버 OS는 `uname -s` 로 판별하고, 답하지 않으면 PowerShell 로 `Win32_OperatingSystem` 을 물어봅니다 — 그 응답 자체가 Windows 라는 증거이면서 `Windows 10 Pro` 같은 에디션 이름도 함께 옵니다.

어댑터가 없는 OS(macOS·BSD)에 연결해도 접속은 됩니다. **파일 탐색과 터미널은 그대로 열어둡니다** — SFTP 와 PTY 는 SSH 가 직접 제공하므로 어댑터가 필요 없기 때문입니다. 나머지 탭은 어떤 OS 로 응답했는지와 함께 이유를 화면에 적습니다.
