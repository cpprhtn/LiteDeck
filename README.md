...
5. **가볍게.** Electron을 쓰지 않습니다. 받는 파일 5~10MB, 설치 후 12~15MB, 실행까지 1초 이내
...
> [!IMPORTANT]
> **아직 만들지 않았습니다.** 이 절만 계획이고, 지금 받는 빌드에는 들어 있지 않습니다.
> README의 나머지는 전부 동작하는 기능입니다.

**MCP 서버 (v0.2 예정).** LiteDeck이 localhost에 MCP 엔드포인트를 열어, Claude Code나
Claude Desktop에서 자연어로 서버를 다루게 하려고 합니다. *"myapp이 500 내는데 원인 봐줘"* 라고
물으면 AI가 서비스 상태·로그·디스크를 스스로 조회해 답하는 그림입니다.

설계에서 정해둔 것:

- **AI는 GUI가 앉던 자리에 앉습니다.** 같은 어댑터, 같은 SSH 연결, 같은 Command Log를 씁니다.
  AI가 실행한 명령도 화면에 `[AI]` 표시와 함께 그대로 흐릅니다
- **서버에는 여전히 아무것도 깔지 않습니다.** AI 도구를 서버에 설치하면 런타임과 상주
  프로세스가 생기고, 컨텍스트 수집용 파일 탐색이 저사양 서버의 I/O를 때립니다.
  그 부하는 전부 고사양 클라이언트가 집니다
- **쓰기는 결재를 거치고, 그 결재 정책은 앱이 갖습니다.** AI 쪽에서 끌 수 없습니다.
  자율 모드가 필요하면 LiteDeck 안에서 켜며, 호스트별로 다르게 둘 수 있습니다
- **읽기 전용부터 냅니다.** 조회만 되는 단계가 먼저이고, 조작은 그 다음입니다
