...
| **Windows·macOS·BSD 서버** | 어댑터 없음. 연결하면 **파일과 터미널만** 동작하고, 나머지 탭은 이유를 화면에 적습니다 |
...
Linux가 아닌 서버에 연결하면 접속 자체는 됩니다 — `uname -s`(없으면 `cmd /c ver`)로 OS를 확인하고, **파일 탐색과 터미널은 그대로 열어둡니다.** SFTP와 PTY는 SSH가 직접 제공하므로 어댑터가 필요 없기 때문입니다. 서비스·프로세스·컨테이너·네트워크 탭은 어떤 OS로 응답했는지와 함께 이유를 적습니다.
